          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              {receipt.customer.country && (
                <p className="text-sm text-muted-foreground">{receipt.customer.country}</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-8 pt-8 border-t">
            {receipt.tax ? (
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-gray-500 uppercase">Amount Breakdown</h3>
                <div className="space-y-3 bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">Subtotal:</span>
                    <span className="text-sm font-medium">{receipt.tax.subtotal.toFixed(2)} USDC</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">
                      {receipt.tax.taxName} ({receipt.tax.taxRate}%) - {receipt.tax.taxCountry}:
                    </span>
                    <span className="text-sm font-medium">{receipt.tax.taxAmount.toFixed(2)} USDC</span>
                  </div>
...
export default function PublicReceiptPage({ params }: { params: { id: string } }) {
  const paymentId = typeof params.id === 'string' ? params.id : params.id[0];
